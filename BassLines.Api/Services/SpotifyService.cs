...
namespace BassLines.Api.Services
{
  public class SpotifyService : ISpotifyService
  {
    ...
    public async Task<List<SpotifyPlaylist>> GetPlaylists(string accessToken, bool basslinesOnly)
    {
      ApplyBearerAuth(accessToken);
      ...
      while (hasMore)
      {
        var result = await (await _spotifyClient.GetAsync($"me/playlists?limit={limit}&offset={page}")).DeserializeHttp<SearchRoot<SpotifyPlaylist>>();

        foreach (var playlist in result.items)
        {
          if (!basslinesOnly || playlist.name.Contains(_spotify.PlaylistPrefix))
          {
        ...
        page++;
        hasMore = result.total > limit * page;
      }

      return playlists.DistinctBy(pl => pl.id).ToList();
    }

    public async Task<SearchRoot<PlaylistTrack>> GetPlaylistTracks(string accessToken, string playlistId, int page)
    {
      ApplyBearerAuth(accessToken);

      const int limit = 25;

      var result = await (await _spotifyClient.GetAsync($"playlists/{playlistId}/tracks?limit={limit}&offset={page * limit}")).DeserializeHttp<SearchRoot<PlaylistTrack>>();

      return result;
    }

    public async Task<HttpStatusCode> RemovePlaylistTrack(string accessToken, string trackUri, string playlistId)
    {
      ApplyBearerAuth(accessToken);

      var response = await _spotifyClient.SendAsync(new HttpRequestMessage
      {
        Method = HttpMethod.Delete,
        RequestUri = new Uri(_spotifyClient.BaseAddress + $"playlists/{playlistId}/tracks"),
        Content = new StringContent(JsonSerializer.Serialize(new { tracks = new List<dynamic>() { new { uri = trackUri } } }), Encoding.UTF8, "application/json")
      });

      return response.StatusCode;
    }

    public async Task<SpotifyPlaylist> CreatePlaylist(string accessToken, CreatePlaylistRequest request)
    {
      ApplyBearerAuth(accessToken);

      var content = new StringContent(JsonSerializer.Serialize(new { name = request.Name, description = request.Description, collaborative = request.Collaborative, @public = request.Public }), Encoding.UTF8, "application/json");

      var response = await _spotifyClient.PostAsync($"users/{request.UserId}/playlists", content);
      ...
      var playlist = await response.DeserializeHttp<SpotifyPlaylist>();

      await AddSongsToPlaylist(accessToken, playlist.id, request.TrackUris);

      return playlist;
    }

    public async Task<HttpStatusCode> UpdatePlaylist(string accessToken, CreatePlaylistRequest request)
    {
      ApplyBearerAuth(accessToken);
      ...
      var response = await _spotifyClient.PutAsync($"playlists/{request.Id}", content);

      return response.StatusCode;
    }

    public async Task AddSongsToPlaylist(string accessToken, string playlistId, string[] trackUris)
    {
      ApplyBearerAuth(accessToken);

      var n = trackUris.Count() / 100;
      var i = 0;
      ...
      while (i <= n)
      {
        uris = trackUris.Skip(i * 100).Take(100).ToList();
        var addTrackResponse = await _spotifyClient.PostAsync($"playlists/{playlistId}/tracks", new StringContent(JsonSerializer.Serialize(new { uris }), Encoding.UTF8, "application/json"));
        i++;
      }
    }
